...
export const getAcceptFriendRequestCallback = (
  io: TalketeerSocketServer,
  socket: TalketeerSocket,
): ClientToServerEvents['acceptFriendRequest'] => {
  return async (userId: string, ack: AckFunc) => {
    ...
    try {
      ...
      if (!sender) throw new Error('Invalid User ID');
      ...
      logger.info(
        `'${receiver._id.toString()}' accepted friend request from '${sender._id.toString()}'.`,
        {
          senderId,
          receiverId,
        },
      ...
      if (!dmRoom) throw new Error('An error occured while creating/reactivating DM room');

      // Save notification
      const notificationObj = await saveNotification(senderId, {
