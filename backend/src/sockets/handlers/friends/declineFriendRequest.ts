...
import { removeFriendObject, getUser } from '@src/services/user.service.js';
import type {
  AckFunc,
...
export const getDeclineFriendRequestCallback = (
  io: TalketeerSocketServer,
  socket: TalketeerSocket,
): ClientToServerEvents['declineFriendRequest'] => {
  return async (userId: string, ack: AckFunc) => {
    ...
    try {
      ...
      logger.info(
        `'${receiver._id.toString()}' declined friend request from '${sender._id.toString()}'.`,
        {
          senderId,
          receiverId,
        },
