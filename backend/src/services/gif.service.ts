...
export class TenorClient {
   ...
   * RType - Response type
   */
  private async fetch<PType extends Record<string, unknown>, RType>(
    url: URL,
    params: PType,
  ): Promise<RType> {
    url.searchParams.append('key', this.apiKey);

    if (this.clientKey) url.searchParams.append('client_key', this.clientKey);
  ...
  public async getFeatured(
    options: Obs<TenorFeaturedParameters> = {
      country: 'US',
      locale: 'en_US',
      contentfilter: 'off',
      limit: 20,
  ...
  public async getCategories(
    options: Obs<TenorCategoriesParameters> = {
      country: 'US',
      locale: 'en_US',
      contentfilter: 'off',
      type: 'featured',
  ...
  public async getSearchSuggestions(
    q: string,
    options: Omit<Obs<TenorSearchSuggestionsParameters>, 'q'> = {
      country: 'US',
      locale: 'en_US',
      limit: 20,
    },
  ...
  public async getAutocomplete(
    ...
    return this.fetch<Omit<Obs<TenorAutocompleteParameters>, 'q'>, TenorAutocompleteResponse>(
      reqURL,
      options,
    );
  }
  ...
  public async getTrendingSearchTerms(
    options: Obs<TenorTrendingSearchTermsParameters> = {
      country: 'US',
      locale: 'en_US',
      limit: 20,
    },
