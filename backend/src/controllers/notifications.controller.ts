...
export const getNotifications = async (req: Request, res: Response) => {
  ...
  const user = await User.findById(userId).populate('notifications').lean().exec();
...
export const getNotification = async (req: Request, res: Response) => {
  ...
  const notification = (user.notifications || []).find(
    (notificationObj) => notificationObj._id.toString() === notificationId,
  );

  if (!notification)
    throw new APIError(
      'No associated notification object found for this ID',
