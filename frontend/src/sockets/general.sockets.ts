...
export function handleSocketConnection(socket: TalketeerSocket, queryClient?: QueryClient) {
  ...
  socket.on('roomDeleted', (roomId, ownerId) => {
    ...
    if (me?.room === roomId) {
      ...
      if (me._id !== ownerId) {
        const owner = queryClient.getQueryData<APIResponse<{ user: IPublicUser }>>([
          'users',
          ownerId,
        ])?.data?.user;
    ...
    if (useRoomsStore.getState().selectedRoomId) useRoomsStore.getState().setSelectedRoomId(null);
  });
    ...
    queryClient?.setQueryData(
      ...
      (old: APIResponse<{ notifications: INotification[] }>) =>
        old
          ? {
              success: true,
              data: { notifications: [...(old.data?.notifications || []), notification] },
            }
          : old,
    );
