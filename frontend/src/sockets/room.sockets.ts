...
export function startListeningRoomEvents(socket: TalketeerSocket, queryClient?: QueryClient) {
  ...
  socket.on('memberKicked', (roomId, userId, kickedBy, reason) => {
    ...
    const room = queryClient?.getQueryData<APIResponse<{ room: IRoom }>>(['rooms', roomId])?.data
      ?.room;
    ...
    if (me?._id === userId) {
      ...
      const admin = queryClient?.getQueryData<APIResponse<{ user: IPublicUser }>>([
        'users',
        kickedBy,
      ])?.data?.user;
  ...
  socket.on('memberBanned', (roomId, userId, bannedBy, banDetails) => {
    console.log(
      `${userId} has been banned from the room (${roomId}) by ${bannedBy} due to the following reason:`,
      banDetails.reason,
    );
    // Invalidate room data to refresh member list
    queryClient?.invalidateQueries({ queryKey: ['rooms', roomId] });
