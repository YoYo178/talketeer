...
export const ChatWindow = () => {
  ...
  const { joinedRoomId, selectedRoomId, setJoinedRoomId, setSelectedRoomId } =
    roomsStore as typeof roomsStore & { joinedRoomId: string };
  ...
  useLayoutEffect(() => {
    if (isInMemberList) setIsInMemberList(false);
  }, [joinedRoomId]);
  ...
  const handleRoomJoin = async () => {
    if (!selectedRoom || selectedRoomId === joinedRoomId || isJoining) return;
      ...
      socket.emit('leaveRoom', joinedRoomId, ({ success }) => {
        if (success) {
          ...
          setTimeout(() => {
            ...
            socket.emit(
              'joinRoom',
              { method: 'id', data: selectedRoom._id },
              ({ success, data }) => {
                if (success) {
      ...
      {selectedRoomId ? (
        <div className='flex flex-col w-full h-full items-center justify-center p-12 gap-2 text-center'>
          ...
          <p className='text-muted-foreground text-sm md:text-base'>
            Members online: {selectedRoom?.memberCount}/{selectedRoom?.memberLimit}
          </p>
          ...
          {selectedRoom?.visibility === 'private' && !selectedRoom?.isSystemGenerated ? (
            selectedRoom?.owner === me?._id ? (
              ...
              <div className='flex flex-col items-center gap-4 text-center'>
                <p>
                  This is a private room, you cannot join unless you are invited by the owner or you
                  have the code for the room.
                </p>
                <Button className='w-fit' onClick={handleClearSelection}>
                  Go back
                </Button>
              </div>
            )
