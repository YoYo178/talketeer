...
export const UserProfilePictureSkeleton = React.memo(() => {
  return (
    <Skeleton className='flex items-center justify-center size-10 min-w-10 max-w-10 aspect-square rounded-full bg-[#d8d8d8] dark:bg-[#242429] self-start' />
  );
});
...
export const UserProfilePicture: FC<UserProfilePictureProps> = ({ user, popoverSide }) => {
  ...
  const handleSendFriendRequest = () =>
    socket.emit('sendFriendRequest', user._id, ({ success }) => success && refreshMeData());
  const handleRevokeFriendRequest = () =>
    socket.emit('revokeFriendRequest', user._id, ({ success }) => success && refreshMeData());

  const handleAcceptFriendRequest = () =>
    socket.emit('acceptFriendRequest', user._id, ({ success }) => success && refreshMeData());
  const handleDeclineFriendRequest = () =>
    socket.emit('declineFriendRequest', user._id, ({ success }) => success && refreshMeData());

  const userNameArray = (user.displayName || user.username).split(' ');
  const userFallbackName = (
    userNameArray.length > 1
      ? [userNameArray[0], userNameArray[userNameArray.length - 1]]
      : [userNameArray[0]]
  )
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button className='flex items-center justify-center size-10 min-w-10 max-w-10 aspect-square rounded-full bg-[#d8d8d8] dark:bg-[#242429] self-start'>
          <Avatar className='rounded-full size-10 object-cover drop-shadow-sm'>
            ...
            <AvatarFallback className='text-primary'>{userFallbackName}</AvatarFallback>
          </Avatar>
        </Button>
      </PopoverTrigger>
      <PopoverContent side={popoverSide} sideOffset={10} asChild>
        <div className='flex flex-col w-fit'>
          ...
          {isFriend && (
            <p className='flex-1 text-center text-muted-foreground text-sm font-semibold pt-2'>
              You're friends with this user!
            </p>
          )}

          {!isSelf && !isFriend && (
            <div className='flex gap-2 pt-4 *:flex-1'>
              {/* Not friends with user, show them 'Add friend' button */}
              {!isPending && <Button onClick={handleSendFriendRequest}>Send friend request</Button>}

              {/* Pending friend request, and is outgoing, meaning we sent them a friend request, show 'Revoke request' button */}
              {isPending && !isIncoming && (
                <Button variant='outline' onClick={handleRevokeFriendRequest}>
                  Friend request sent
                </Button>
              )}
