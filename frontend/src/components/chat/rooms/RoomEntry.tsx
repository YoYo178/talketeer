...
export const RoomEntry: FC<RoomEntryProps> = ({ room: localRoom }) => {
  ...
  const handleRoomJoin = async () => {
    if (dmRoomId) setDmRoomId(null);

    if (!selectedRoom || selectedRoomId === joinedRoomId || isJoining || !me) return;

    if (selectedRoom.visibility === 'private' && selectedRoom.owner !== me._id) return;
  ...
  return (
    ...
    >
      <div className='flex items-center gap-2'>
        {room.visibility === 'private' && <Lock className='text-muted-foreground' size={16} />}
        <h1 className='text-sm md:text-base font-medium'>{room.name}</h1>

        {room.isSystemGenerated && (
          <span className='text-[8px] md:text-[10px] px-1.5 py-0.5 rounded-md bg-secondary-foreground text-primary-foreground font-bold'>
            SYSTEM
          </span>
        )}
      </div>
      {!room.isSystemGenerated && (
        <span className='text-xs text-muted-foreground'>
          Created by: <span className='font-semibold'>{`@${owner?.username}`}</span>
        </span>
      )}
      <div className='text-xs text-muted-foreground'>
        Members: {room.memberCount}/{room.memberLimit}
      </div>
    </button>
  );
};
