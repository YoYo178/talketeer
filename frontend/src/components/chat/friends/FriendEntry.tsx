...
export const FriendEntry: FC<FriendEntryProps> = ({ friendObject }) => {
  ...
  const handleAcceptFriendRequest = () =>
    socket.emit('acceptFriendRequest', user._id, ({ success }) => {
      successCallback({ success });

      if (dmRoomId) queryClient.invalidateQueries({ queryKey: ['dm-rooms', dmRoomId] });
    });

  const handleDeclineFriendRequest = () =>
    socket.emit('declineFriendRequest', user._id, successCallback);
  const handleRevokeFriendRequest = () =>
    socket.emit('revokeFriendRequest', user._id, successCallback);

  const handleRemoveFriend = () =>
    socket.emit('removeFriend', user._id, ({ success }) => {
      successCallback({ success });
  ...
  const handleSendMessage = () => {
    if (!dmRoom || dmRoomId === dmRoom._id) return;
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription asChild>
                    <p>
                      You are about to remove <span className='font-bold'>@{user.displayName}</span>{' '}
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction asChild>
                    <Button
                      className='text-primary bg-red-600 hover:bg-red-500'
