    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <div className='relative'>
          <Bell className='cursor-pointer min-w-6 size-6' onClick={() => setIsOpen(!isOpen)} />
          {hasNewNotifications && (
            <div className='absolute top-0 right-0 rounded-full size-2 bg-red-600 outline-2 outline-primary' />
          )}
        </div>
      </PopoverTrigger>
      <PopoverContent align='end' side='bottom' alignOffset={10} sideOffset={10} asChild>
