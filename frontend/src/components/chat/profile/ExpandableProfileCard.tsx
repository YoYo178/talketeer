...
export const ExpandableProfileCard = () => {
  ...
  const fallbackName = (
    nameArray.length > 1 ? [nameArray[0], nameArray[nameArray.length - 1]] : [nameArray[0]]
  )
      ...
      >
        {!me.displayName ? (
          ...
          <div className='flex flex-col'>
            <p
              className={`text-sm font-semibold leading-tight ${isAvatarCardOpen && hasCardTransitionFinished ? '' : 'text-nowrap'}`}
            >
              {me.displayName}
            </p>
            <p
              className={`text-xs text-muted-foreground leading-tight ${isAvatarCardOpen && hasCardTransitionFinished ? '' : 'text-nowrap'}`}
            >
              @{me.username}
            </p>
          </div>
