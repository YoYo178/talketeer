...
export const ChatMemberList = () => {
  const { isMemberListOpen, setIsMemberListOpen, isSidebarOpen, setIsSidebarOpen } =
    useGlobalStore();
  ...
  const handleMemberListToggle = () => {
    if (!isLargeScreen && isSidebarOpen) setIsSidebarOpen(false);
  ...
  return (
    <div
      className={`flex rounded-xl bg-accent dark:bg-primary-foreground transition-[padding-inline] duration-300 ${isMemberListOpen ? 'pr-3' : ''}`}
    >
      <button
      ...
      <div
        className={`w-86 my-4 flex flex-col gap-6 overflow-hidden transition-[opacity,max-width] duration-300 ${isMemberListOpen ? 'opacity-100 max-w-86' : 'opacity-0 max-w-0'}`}
      >
