...
export const MessageText: FC<MessageTextProps> = ({ content, isSelfMessage }) => {
    ...
    return (
      ...
      >
        {parts.map((part, j) => {
          ...
          return (
            <span key={`part-${i}-${j}`} className='text-sm md:text-base'>
              {part}
            </span>
          );
        })}
  ...
  const embeddableElements = lines.flatMap((line, i) => {
    return line
      ...
      .map((part, j) => {
        ...
        const isTenorLink = supportedEmbedOrigins.some(
          (origin) => part.startsWith(origin) && part.endsWith('.gif'),
        );
  ...
  return (
    <div className={`w-full flex flex-col`}>
      <div
        className={`w-full flex flex-col ${isSelfMessage ? 'items-end text-end' : 'items-start text-start'}`}
      >
        {messageElements}
      </div>
      ...
      {embeddableElements.length > 0 && (
        <div
          className={`w-full flex flex-col gap-2 ${isSelfMessage ? 'items-end text-end' : 'items-start text-start'}`}
        >
          {embeddableElements}
        </div>
      )}
