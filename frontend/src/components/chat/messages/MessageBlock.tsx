...
export const MessageBlockSkeleton: FC<MessageBlockSkeletonProps> = ({ align }) => {
  const generateArbitraryLines = (
    ...
    return Array.from({ length: lines }).map((_, i) => {
      ...
      return <MessageTextSkeleton key={i} height={1} width={lineLength} />;
    });
  };
  ...
  return (
    <div
      className={
        `flex self-${align} gap-2 p-2 pb-1 mb-2 overflow-hidden` +
      ...
      {isSelfMessage ? (
        <>
          <div className='flex flex-col items-end w-full'>
            <p className='text-muted-foreground text-sm'>
              ...
              <span className='text-[#383838] dark:text-[#696969] text-sm font-extrabold ml-2 mr-2'>
                ——
              </span>
              <span className='font-semibold'>You</span>
            </p>
            {messages.map((message) => (
              <MessageText key={message._id} content={message.content} isSelfMessage />
            ))}
          </div>
        ...
        <>
          ...
          <div className='flex flex-col items-start w-full'>
            ...
            {messages.map((message) => (
              <MessageText key={message._id} content={message.content} />
            ))}
          </div>
        </>
      )}
