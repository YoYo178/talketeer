...
export const MessageList = () => {
  ...
  const messagePages = useMemo(() => [...pages].reverse(), [pages]);
  ...
  const handleScroll = useCallback(
    (e: React.UIEvent<HTMLDivElement>) => {
      ...
      if (
        element.scrollTop === 0 &&
        (dmRoomId ? dmMessagesQuery.hasNextPage : messagesQuery.hasNextPage)
      ) {
    ...
    [
      dmRoomId,
      dmMessagesQuery.hasNextPage,
      messagesQuery.hasNextPage,
      dmMessagesQuery.fetchNextPage,
        ...
        {(dmRoomId ? dmMessagesQuery.isLoading : messagesQuery.isLoading) ? (
          ...
          <div className='text-center'>
            <p className='text-base text-muted-foreground md:text-xl'>
              {dmRoomId
                ? 'This conversation is currently empty.'
                : 'This room currently has no messages.'}
            </p>
  ...
  return (
    <ScrollArea
      className='flex flex-col flex-1 p-4 pt-0 pb-0 overflow-y-auto overflow-x-hidden'
          ...
          (dmRoomId ? !dmMessagesQuery.hasNextPage : !messagesQuery.hasNextPage) && (
            <div className='flex items-center my-2'>
              ...
              <span className='mx-2 bg-transparent text-center px-2 dark:text-muted-foreground text-sm md:text-base'>
                {dmRoomId
                  ? `This is the beginning of your conversation with @${friendUser?.username}`
