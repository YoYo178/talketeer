...
export const ChatComposer = () => {
  ...
  const dmTypingUsers = dmRoomId
    ? typingUsers.filter((usr) => usr.roomType === 'dm' && usr.roomId === dmRoomId)
    : [];
  const roomTypingUsers = joinedRoomId
    ? typingUsers.filter((usr) => usr.roomType === 'normal' && usr.roomId === joinedRoomId)
    : [];
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (dmRoomId) {
      ...
      if (typingTimer.current) {
        ...
        typingTimer.current = null;
      } else socket.emit('startDmTyping', me._id, dmRoomId, me.username);
      ...
      if (typingTimer.current) {
        clearTimeout(typingTimer.current);
        typingTimer.current = null;
      } else socket.emit('startTyping', me._id, joinedRoomId, me.username);
  ...
  const sendMessage = () => {
    ...
    if (typingTimer.current) {
      ...
      if (dmRoomId) socket.emit('stopDmTyping', me._id, dmRoomId);
      else if (joinedRoomId) socket.emit('stopTyping', me._id, joinedRoomId);
    }
  };
